...
outcomes %>% 
  mutate(
    which_responses = glue('{which_responses} responses'),
    which_rules = glue('{which_rules} rules'),
    which_responses = factor(which_responses, levels = c('own responses','coplayer responses')),
    which_rules = factor(which_rules, levels = c('regular rules','irregular rules','both rules'))
  ) %>% 
  ggplot() +
  geom_smooth(aes(weight,mean_reg), method = 'loess', colour = 'lightgreen') +
  ...
  # ggtitle('Tuning rules\non the ESP task') +
  facet_wrap( ~ which_responses + which_rules, ncol = 3) +
  coord_cartesian(ylim = c(.25,.65), xlim = c(.15,1)) +
  guides(colour = 'none') + plot_annotation(title = 'Figure 5. Updated rule confidence and predictive accuracy in the posttest.')
```
